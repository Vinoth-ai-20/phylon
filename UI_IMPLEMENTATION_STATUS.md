...
| R-3 | `plugins/navigation.rs::navigation_ui` is a permanent no-op stub | Never wired to anything; unclear if it's a placeholder for a future nav breadcrumb feature or should be deleted | `crates/ui/src/plugins/navigation.rs:8-15` | Low | Low — decide "delete" vs. "implement" rather than leave ambiguous |
| R-4 | `utils.rs::draw_segment_tree` has no call site | Same class of issue as R-3 | `crates/ui/src/utils.rs:83` | Low | Low |
