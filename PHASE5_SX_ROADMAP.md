...
| **SX-2d** | Developmental growth visual: a segment "forming" transition when `growth_system` spawns a new node (P4-F1's persistent graph already knows exactly when this happens) | 2 | Low |

### Epic 3 — Ecological Storytelling (depends on: SX-1e, Epic 1's visual language existing to build on)

| Milestone | Goal | Effort | Risk |
|---|---|---|---|
| **SX-3a** | Close the event-coverage gap in §2.6: wire `ReproductionEvent`/`FieldSpike` to real consumers (narration + `TimedEffects`), or explicitly retire them if truly unneeded — no more silently-dead event variants | 2 | Low |
