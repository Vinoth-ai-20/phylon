...
- **Inspector:** Genetics section's existing `MutationCount`/`MutationHistory` fields (currently hardcoded "Not Available" — a real gap, see SX-4b) should show this organism's actual distance-from-parent value.
- **Chart:** none new.
...
- **Inspector:** Neural section's currently-hardcoded "Not Available" fields (`CTRNNState`, `BrainOutputs`, etc.) should show real live values — closes a confirmed gap (§2.3 of the main roadmap audit), required before this entry's viewport glyph can be considered complete (the numbers must exist before they can drive a glyph).
- **Chart:** none.
- **Event log:** none.
