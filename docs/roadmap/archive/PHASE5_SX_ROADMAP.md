...
| Milestone | Goal | Effort | Risk |
|---|---|---|---|
| **SX-7a** | **Done — see §11 (bundled, per your explicit request).** `NarrationLog` (not `PhylonEvent` directly — see re-audit) plotted as `VLine` markers on the Demographics plot; fixed a real `tick: 0` placeholder bug in `process_narrative_events_system` found while wiring this | 3 | Low |
| **SX-7b** | **Done — see §11 (bundled).** Per-series checkboxes + running-mean overlay on Demographics/Diversity, backed by a new `MetricsSeriesOptions` in `WorkbenchState` | 3 | Medium |
| **SX-7c** | **Done — see §11 (bundled).** Per-chart "Export PNG" button on all 7 Metrics charts — crops the existing whole-window screenshot GPU readback to one chart's rect rather than a new rendering path | 2 | Low |

### Epic 8 — Visual Hierarchy (depends on: ADR-P5-05)

| Milestone | Goal | Effort | Risk |
|---|---|---|---|
| **SX-8a** | **Done — see §11 (bundled, per your explicit request).** New `theme::CHROME_TITLE_CONTEXTUAL`/`CHROME_TITLE_SECONDARY`/`CHROME_ACCENT_BAR` tokens + a `PanelTier` enum in `layout.rs` — Viewport's Primary tier needs no token since it never reaches `chrome_bar` at all | 2 | Low |
| **SX-8b** | **Done — see §11 (bundled).** Applied inside the single already-consolidated `chrome_bar` (covers all 14 panels + tabbed/floating variants in one place); fixed the real Evolution Debugger default-visibility bug in both Research and Presentation presets | 2 | Low |
| **SX-8c** | **Done — see §11 (bundled).** New "Behavior" status-bar zone — Hunting/Diseased counts queried from the same `BehaviorState`/`Infection` components SX-1b/1d already added | 2 | Low |

### Epic 9 — Simulation Experience / Onboarding (depends on: Epics 1, 6, 8 substantially landed)

| Milestone | Goal | Effort | Risk |
|---|---|---|---|
| **SX-9a** | **Done — see §11 (bundled, per your explicit request).** One dismissible "Welcome to Phylon" dialog (not a tour) — 4 viewport signals, 1 Inspector pointer — shown once per session on entering the simulation, re-openable via Help → Welcome Tips | 2 | Low |
| **SX-9b** | **Done — see §11 (bundled).** Full pass against all 5 success criteria in §9 — 4 satisfied, 1 (Epic 2 motion) honestly reported as partially satisfied, citing ADR-P5-07's own already-disclosed 7.8%-vs-90.7% gap rather than re-litigating or hiding it | 1 | Low |
...
### SX-7a/7b/7c — Scientific Visualization (bundled, per your explicit "proceed Epic 7 as a bundle")

**Re-audit before implementing:** `metrics.rs` (the Metrics Dashboard) was already a 2-column, 6-plot `egui_plot` grid with hand-picked `theme::CHART_*` colors and static `chart_legend_dot` legend rows — no toggles, no annotations, no export beyond the existing CSV/JSON buttons (Phase 2, M14). Three things the roadmap's own wording assumed turned out to need correction or new investigation:

- **The roadmap named `PhylonEvent` as a possible annotation source; it isn't the right one.** `events::PhylonEvent` (`crates/events/src/lib.rs`) has exactly four variants — `OrganismBorn`, `OrganismDied`, `ReproductionEvent`, `ExperimentCheckpoint` — none of which is a population-level "hazard/outbreak/extinction" milestone. `analytics::NarrationLog` (already consumed by the Event Log panel) is the actual fit: its own doc comment says its purpose is "contextualizing why a population graph suddenly dropped" — the exact SX-7a use case, word for word. Annotations read from `NarrationLog`, not `PhylonEvent`.
- **A real, previously invisible bug in the data SX-7a was about to plot.** `process_narrative_events_system` (`crates/app/src/systems.rs`) has logged every hazard event at a hardcoded `tick: 0 // TODO` since it was written — harmless while nothing read `NarrativeEvent::tick`, but it would have put every hazard marker at x=0 on the Demographics plot once SX-7a started rendering them as `VLine`s. Same class of bug SX-3a fixed twice for `ReproductionEvent`. Fixed by adding `Res<metabolism::GlobalAtmosphere>` to the system and logging `atmosphere.ticks` (the same tick counter `expire_timed_effects_system` already reads for the same reason), with a new regression test (`hazard_event_logs_the_real_tick_not_a_hardcoded_zero`) proving it.
- **Annotations were scoped to the Demographics plot only, not all six.** The roadmap said "Metrics' time axis" generically; `NarrationLog`'s own doc comment ties it specifically to population-graph context, and Hazard/Predation/Lineage markers on the Performance/Resources/Environment/Colony Connectivity plots would be visual noise unrelated to what those plots show. Disclosed narrowing, not silently expanded scope in the other direction.

For SX-7c, the roadmap's "Image export (PNG)" didn't specify a mechanism. Two were available: draw the chart data a second time through a standalone plotting library (e.g. `plotters`), or reuse the existing whole-window screenshot GPU readback (`crates/app/src/capture.rs`, already wired to `MenuAction::TakeScreenshot`) cropped to one chart's rect. Chose the latter — zero new dependencies, and the readback already runs after the egui pass (so it includes the chart pixels as rendered, axis labels and all) at the exact timing point `TakeScreenshot` already established.

**Implementation:**

- **SX-7a:** `metrics_ui` now reads `NarrationLog` and converts each event's `tick` to the same `sim_time` (seconds) unit the plots' X axis already uses, via `tick as f64 * TickRate::dt()` — valid because `MetricsState::sim_time` and `GlobalAtmosphere::ticks` both advance once per simulation tick by the same per-frame `ticks_to_run` (`crates/app/src/render.rs`), so no new plumbing was needed to keep the units aligned. Each entry becomes an `egui_plot::VLine` on the Demographics ("pop_plot") plot, colored by `event_type` via the already-existing `theme::WARN`/`BAD`/`GOOD` tokens (Hazard/Predation/Lineage respectively, `ACCENT` as a fallback) — no new color tokens, reusing what a prior UI-token milestone already added.
- **SX-7b:** added `state::MetricsSeriesOptions` (per-series `bool` visibility for the 5 Demographics + 4 Diversity series, plus a running-mean toggle per plot) to `WorkbenchState` — pure display preference, not simulation data, consistent with the "never cache simulation data in `WorkbenchState`" rule (this isn't simulation data being cached, it's a UI toggle). `metrics.rs`'s static legend rows became `demographics_toggle` (a `chart_legend_dot` + checkbox composite, not a second hand-rolled legend renderer) driving which lines actually render. A new free function `running_mean` (`crates/ui/src/plugins/metrics.rs`) computes a trailing `RUNNING_MEAN_WINDOW`-sample (10) average, rendered as a dashed overlay (`LineStyle::Dashed`) in the same series color when enabled — deliberately extracted as a standalone function (not an inline closure) so it has its own unit tests independent of egui/`MetricsState`.
- **SX-7c:** every one of the 7 Metrics charts (Demographics, Performance, Diversity, Resources, Environment, Colony Connectivity, Species Distribution) now has a small "Export PNG" button below it, capturing that specific `egui_plot::Plot`'s response rect. A new `MenuAction::ExportChartPng { x, y, width, height }` (physical pixels, converted via `ctx.pixels_per_point()` at the call site) carries the crop region; `App::pending_chart_export` defers the actual capture to the same next-`render()` timing point `pending_screenshot` already uses (must run before `output.present()`, after the egui pass). `capture::save_chart_png` crops the whole-window readback via `image::imageops::crop_imm` and saves under `./screenshots/phylon_chart_<timestamp>.png` — same destination as regular screenshots, since a chart export is just a narrower crop of the same artifact type, not a new one.

**Verification:** `cargo build --workspace --all-targets`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --all -- --check` — all clean. `cargo test --workspace` — 0 failures across every crate, including 4 new tests: `hazard_event_logs_the_real_tick_not_a_hardcoded_zero` (proves the tick-bug fix) and three `running_mean_*` tests (clamped-window-before-full-history, trailing-window-only averaging past a step change, and X-value preservation) — run once across the full bundle rather than three times.

**Disclosed limitations, not glossed over:**

- No automated test for the VLine-rendering or PNG-export code paths themselves (egui/GPU-readback side) — consistent with every prior UI-panel milestone this phase; only the pure-logic pieces (`running_mean`, the tick fix) got unit tests, per this project's established "no meaningful unit-test surface for egui rendering itself" testing strategy.
- No screen-capture/automation driver available this session — whether the VLine markers are visually legible against the existing 5-series Demographics plot without clutter, whether the running-mean dashed overlay is visually distinguishable from the raw line at typical zoom, and whether the PNG crop rect lands pixel-accurately on every plot (especially under a non-1.0 `pixels_per_point` / HiDPI scale factor) are all unconfirmed without an actual viewer's reaction.
- The PNG export requires one extra rendered frame after the button click (the crop happens against *the next* `render()` call, same as `TakeScreenshot`) — if the user immediately changes the plot's zoom/pan or the data updates between click and capture, the exported PNG could reflect a slightly different frame than what was on screen at click time. Same timing behavior `TakeScreenshot` already has; not a new risk introduced here, but worth naming since SX-7c is the first per-chart (not per-window) use of this pattern.
- `analytics` crate's `Cargo.toml` declares an `egui_plot` dependency that nothing in `analytics/src` actually uses (confirmed via grep) — a pre-existing dead dependency, noticed while auditing this epic but out of scope to remove here (untouched by this milestone's files).

**Stopping here, per Implementation Discipline.** Epic 7 (Scientific Visualization) is now complete: SX-7a, SX-7b, SX-7c, all bundled per your explicit request. Waiting for approval before starting the next epic (Epic 8 — Visual Hierarchy, per the roadmap's own ordering).

### SX-8a/8b/8c — Visual Hierarchy (bundled, per your explicit "proceed Epic 8 as a bundle")

**Re-audit before implementing:** ADR-P5-05 (§5) calls for a three-tier chrome system — Primary (Viewport), Contextual (Inspector/selection-dependent panels), Secondary (Metrics/Dashboards/Logs) — applied to `layout.rs`'s existing `panel_chrome`/`floating_chrome`. Re-reading `layout.rs` before touching it turned up two things worth flagging:

- **The chrome-bar consolidation ADR-P5-05 assumes was already done.** §2.4/§5's wording (and the earlier UI-audit context this roadmap was born from) describes `panel_chrome`, `WorkbenchBehavior::top_bar_right_ui`, and `floating_chrome` as three independent, drifted implementations. They aren't, anymore — all three already call one shared `chrome_bar` function (with matching `CLOSE_RED`/`DETACH_BLUE` tokens), predating this epic. This meant SX-8a/8b's actual work was smaller than the ADR implies: one function to add tiering to, not three to reconcile first.
- **§2.4's Evolution Debugger bug is real and still present.** `apply_layout_preset`'s `LayoutPreset::Research` arm closes Neural Viewer/Research Dashboard/Replay Browser/the five P4-R-tier viewers/Placeholder Panel — but not Evolution Debugger — while a comment a few dozen lines away (`rebuild_tree_from_modes`, around the `evolution_debugger` tile) says it gets "the same treatment as Research Dashboard/Replay Browser... default to Closed." The code and the comment already disagreed before this session touched either.

**Implementation:**

- **SX-8a:** three new `theme.rs` tokens — `CHROME_TITLE_CONTEXTUAL` (full-strength title text), `CHROME_TITLE_SECONDARY` (dimmed one step), `CHROME_ACCENT_BAR` (reuses `ACCENT`, no new color introduced). A private `PanelTier` enum + `panel_tier(name: &str)` classifier in `layout.rs` — Contextual for Sidebar (hosts Inspector), Neural Viewer, and the five P4-R-tier viewers (Physiology/Circulation/Hormone/Immune/Cell Lineage); Secondary for everything else (Metrics, Event Log, Research Dashboard, Replay Browser, Evolution Debugger, Placeholder Panel). Viewport gets no tier at all, since `panel_chrome` returns early for it before `chrome_bar` is ever reached — Primary's distinguishing feature was already "no chrome," which the ADR's own tiering doesn't need to re-implement.
- **SX-8b:** applied inside `chrome_bar` itself — a small colored rect drawn left of the title only for the Contextual tier (a structural cue, not just color) plus the tier-appropriate title color. Because `chrome_bar` is the one function all three chrome variants (docked, tabbed, floating) already call, this covers all 14 panels in one place, not 14 separate edits. Deliberately a color + accent-bar distinction, not a size change — the ADR explicitly warns against just resizing `CHROME_HEIGHT`. Also fixed the real bug found in re-audit: `Evolution Debugger` now closes in both `LayoutPreset::Research` and `LayoutPreset::Presentation`, matching what the `rebuild_tree_from_modes` comment already (and now correctly) claims.
- **SX-8c:** a new "Behavior" zone in `status_bar.rs`, between the existing Population and System zones — "Hunting: N" and "Diseased: M" chips. Queries `behavior::BehaviorState` (filtered to `Hunting`) and `ecology::disease::Infection` (filtered to `InfectionState::Infectious`) directly — the same components SX-1b/1d already added for the population-wide viewport glyph/tint, aggregated into a count here rather than introducing a second data path. The Diseased count's label tints `theme::WARN` only when nonzero (zero reads as the normal text color), so the zone doesn't demand attention when there's nothing to report.

**Verification:** `cargo build --workspace --all-targets`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --all -- --check` — all clean. `cargo test --workspace` — 0 failures across every crate, including 4 new tests in `layout.rs`: `research_preset_closes_evolution_debugger`, `presentation_preset_closes_evolution_debugger`, `debug_preset_leaves_evolution_debugger_docked` (proves the bug fix in all three presets, including that Debug is correctly unaffected), and `panel_tier_distinguishes_contextual_from_secondary` (spot-checks the tier classification, not exhaustive over all 14 names since it's a static match rather than derived logic that could drift).

**Disclosed limitations, not glossed over:**

- No automated test for the actual chrome-bar rendering (accent-bar rect, title color) — consistent with this project's established "no meaningful unit-test surface for egui rendering itself" testing strategy; only the pure-logic pieces (`panel_tier`, the preset fix) got unit tests.
- No screen-capture/automation driver available this session — whether the Contextual accent bar and dimmed Secondary title actually read as a clear hierarchy at a glance, rather than a subtle difference nobody notices, is unconfirmed without an actual viewer's reaction. This is the same limitation disclosed at the end of every UI-visual milestone this phase.
- Tabbed panes (Metrics/Event Log, inside the shared `Tabs` container) don't get the tier-colored *title* text, since `egui_tiles`' own tab-strip widget draws that title, not `chrome_bar` — only the accent-bar/no-accent-bar distinction reaches them via `top_bar_right_ui`'s `chrome_bar` call (both are Secondary, so no accent bar shows either way — no visible regression, but the title-color half of the tiering doesn't extend into the tab strip itself. Out of scope to fix here without a deeper `egui_tiles` styling-hook investigation than this milestone's effort budget allows.
- SX-8c's "Hunting"/"Diseased" counts are two specific aggregates the roadmap named explicitly; other `BehaviorState` variants (Fleeing, Foraging, Mating, Sleeping) and `Health`-critical counts were considered but not added, to avoid growing the status bar beyond what the roadmap actually asked for.

**Stopping here, per Implementation Discipline.** Epic 8 (Visual Hierarchy) is now complete: SX-8a, SX-8b, SX-8c, all bundled per your explicit request. Waiting for approval before starting the next epic (Epic 9 — Simulation Experience / Onboarding, per the roadmap's own ordering).

### SX-9a/9b — Simulation Experience / Onboarding (bundled, per your explicit "proceed Epic 9 as a bundle")

**Re-audit before implementing:** no onboarding/first-run/tour infrastructure existed anywhere in `crates/ui/src` before this milestone (confirmed via grep across the crate) — this was genuinely new, not a gap in an existing mechanism. `dialogs.rs` (About/Documentation/Keybinds) was the closest existing pattern: one `egui::Window` per dialog, gated by a `bool` field on `WorkbenchState`, all rendered from a single `show_dialogs` entry point already called every frame from `render.rs`. Also confirmed `show_dialogs` renders during *both* `AppState::MainMenu` and `AppState::Simulation` (`render.rs:57-65`) — meaning a naive `show_onboarding_hints: true` default at `WorkbenchState::default()` construction would show a dialog referencing "the viewport" and "the Inspector" over the Main Menu screen, before either exists. Sequenced instead off `MenuAction::StartSimulation`'s handler (`crates/app/src/events.rs`), which already resets `show_about`/`show_docs` at the exact moment the user transitions from Main Menu into the simulation view — the natural, already-existing "first real frame of the simulation" hook.

**Implementation:**

- **SX-9a:** new `WorkbenchState::show_onboarding_hints: bool` (defaults `false` at construction, set `true` by `StartSimulation`'s handler, per the re-audit above). New `dialogs::onboarding_hints_dialog` — one non-modal, dismissible `egui::Window` ("Welcome to Phylon"), explicitly *not* a multi-step tour per the roadmap's own wording: 4 rows under "In the viewport" (behavior glyphs — SX-1b; health-brightness outline — SX-1c; disease tint — SX-1d; birth/death floating text — SX-1e/SX-2c) and 1 row under "In the Inspector" (click-to-select, then expand the new Physiology/Circulation/Hormones/Immune/Evolution-History sections — Epic 6), each using the *exact* icon+color pairs `render_behavior_glyphs`/`theme::GOOD`/`WARN`/`BAD` already use, not new, freely-invented ones. A "Got it" button and re-openable afterward via a new Help → "Welcome Tips" menu entry (`MenuAction::ShowOnboardingHints`), mirroring the existing About/Keybinds re-open pattern exactly.
- **SX-9b:** a documentation-only audit (no code) — re-read every Epic 1-8 execution-log entry in this file and checked each of §9's 5 success criteria against what's actually implemented and disclosed, not re-measured from scratch (no screen-capture driver exists this session, same standing limitation as every prior milestone; this pass is a code/documentation cross-check, not a new user study). Results:
  - **"What organisms are doing" (Epic 1/2) — partially satisfied, honestly, not overstated.** `BehaviorState` glyphs (SX-1b) make every organism's current activity legible population-wide, unconditionally. But the underlying capacity to actually *move* remains limited: ADR-P5-07's own final measurement (quoted directly, not re-run) found only **7.8% of founding organisms have at least one actuatable effector spring** post-fix, up from 0.3% pre-fix but still well below the 90.7% isolated/per-position rate — a gap ADR-P5-07 itself flagged as unexplained and unresolved. This criterion is satisfied for *behavioral state* (always legible) but only partially for *physical motion* (still uncommon population-wide) — reported as-is, not smoothed over.
  - **"Which organisms are active/interesting right now" (Epic 1/5) — satisfied.** Health-brightness (SX-1c) and disease-tint (SX-1d) are always-on population-wide signals; Spotlight (SX-5b), organism labels (SX-5a), and trajectory trails (SX-5c) give an opt-in path to follow a specific one once found.
  - **"Where ecological events are occurring" (Epic 1/3) — satisfied.** Floating text at the organism's own position marks every death (with cause, SX-1e) and predation moment (SX-2c); `NarrationLog` hazard/lineage milestones are logged with a real tick (SX-3a, and the tick-bug fix that made SX-7a's own annotations trustworthy) and now also appear directly on Metrics' Demographics time axis (SX-7a).
  - **"Which organisms deserve attention" (Epic 4/5) — satisfied.** Inspector no longer dead-ends into generic "Not Available" placeholders for a nonexistent entity (SX-4a), missing neural/mutation data (SX-4b), relationship/trajectory context (SX-4c), or species population (SX-4d); Spotlight/labels (Epic 5) surface candidates before a click is even made.
  - **"Why populations are changing" (Epic 3/7) — satisfied.** Species distribution snapshot (SX-3b) and lineage ancestor/descendant chains (SX-3c, with its own disclosed asymmetry) explain composition; Metrics' event annotations and running-mean overlay (SX-7a/7b) directly connect a population swing to a specific, dated cause.

  **Net result: 4 of 5 criteria satisfied, 1 partially satisfied** (physical motion prevalence, a pre-existing, already-disclosed limitation from Epic 2's own execution log, not a new finding this milestone). Not rounded up to "5 of 5" — the roadmap's own standing rule is to report limitations honestly, and this criterion's gap was already measured and named in ADR-P5-07, so repeating it here rather than silently dropping it is the correct disclosure, not a new problem introduced by SX-9b.

**Verification:** `cargo build --workspace --all-targets`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --all -- --check` — all clean. `cargo test --workspace` — 0 failures across every crate (SX-9a added no new automated tests — a dialog's rendering/dismissal flow has no unit-test surface without a live `App`/window, the same limitation every prior dialog in this file already has, and SX-9b is a documentation-only audit with nothing to unit test).

**Disclosed limitations, not glossed over:**

- SX-9a's "first-run" is session-scoped, not persisted across app restarts — this codebase has no settings/preferences persistence mechanism at all (confirmed via grep for `confy`/settings-file patterns, found none), and building one would be a materially separate initiative, not a Low-effort addition to an onboarding hint. Every session, the dialog shows once on entering the simulation; a returning user sees it again unless they've kept the process running.
- No screen-capture/automation driver available this session — whether the onboarding dialog's wording and icon/color choices actually read clearly to a first-time viewer (the dialog's whole purpose) is unconfirmed without an actual viewer's reaction, the same limitation disclosed at the end of nearly every UI milestone this phase.
- SX-9b's audit is a re-check of this document's own prior, already-honest disclosures — it does not re-run ADR-P5-07's headless diagnostic or re-verify any earlier milestone's numbers from scratch. If a prior milestone's self-reported result were wrong, this audit would not catch that; it cross-references what's already written, it doesn't re-measure.

**Stopping here, per Implementation Discipline.** Epic 9 (Simulation Experience / Onboarding) is now complete: SX-9a, SX-9b, both bundled per your explicit request. This closes the last epic in the roadmap's §6 Milestone Roadmap. Waiting for your direction on what's next — whether that's the deferred Epic 2 motion-prevalence gap (the one partially-satisfied success criterion), a fresh audit pass, or something outside this roadmap entirely.
