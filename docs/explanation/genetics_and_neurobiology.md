...
- When an organism is born, its CPPN is queried for every possible pair of nodes. If the CPPN outputs a non-zero value, a synapse is created. The evaluation loop cleanly isolates structural `Input` nodes, preventing coordinate erasure.
