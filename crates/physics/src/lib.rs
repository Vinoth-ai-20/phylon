...
pub fn circle_circle_collision(world: &mut World, grid: &spatial::UniformGrid, restitution: f32) {
    ...
    for (entity_a, (pos_a, vel_a, radius_a, mass_a)) in world.query::<(&Position, &Velocity, &Radius, &Mass)>().iter() {
        ...
        for dx in -1..=1 {
            for dy in -1..=1 {
                ...
                for &entity_id_b in grid.query_cell(neighbor_cell) {
                    if id_a.0 >= entity_id_b.0 { continue; } // Avoid double-checking and self-checking
                    let entity_b = hecs::Entity::from_bits(entity_id_b.0).unwrap();
                    
                    if let Ok(mut q_b) = world.query_one::<(&Position, &Velocity, &Radius, &Mass)>(entity_b) {
                        if let Some((pos_b, vel_b, radius_b, mass_b)) = q_b.get() {
                            ...
                            if dist_sq < combined_radius * combined_radius && dist_sq > 0.0001 {
                                ...
                                *displacements.entry(entity_a).or_insert(Vec2::ZERO) -= normal * overlap * ratio_a;
                                *displacements.entry(entity_b).or_insert(Vec2::ZERO) += normal * overlap * ratio_b;
                                ...
                                if vel_along_normal < 0.0 {
                                    ...
                                    *velocity_changes.entry(entity_a).or_insert(Vec2::ZERO) -= impulse / mass_a.0;
                                    *velocity_changes.entry(entity_b).or_insert(Vec2::ZERO) += impulse / mass_b.0;
                                }
                            }
                        }
                    }
