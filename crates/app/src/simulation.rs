...
use crate::biology_profiler::BiologyCategory;
use crate::systems::*;
...
/// Times a per-tick biology system call for the P9.1b hierarchical
/// profiler (`crate::biology_profiler`) when it's enabled — near-zero cost
/// otherwise (one `bool` check, no `Instant::now()`, no map access). This
/// is a macro rather than a method because the wrapped `$body` needs to
/// mutably borrow the same `self` the profiler bookkeeping also reads
/// (`self.world.ecs...` inside `$body`, `self.biology_profiler_state`
/// after it) — inlining avoids the borrow-checker conflict a closure- or
/// method-based wrapper taking `&mut self` twice would hit.
///
/// Reads the shared per-tick entity-count context from
/// `self.biology_profiler_population` (set once per tick in
/// `update_simulation`, only when the profiler is enabled) — see
/// `biology_profiler`'s module doc for why this is one shared count, not a
/// category-specific filtered one.
macro_rules! profiled {
    ($self:ident, $category:expr, $body:expr) => {{
        if $self.biology_profiler_config.enabled {
            let _profiled_start = std::time::Instant::now();
            $body;
            let _profiled_ms = _profiled_start.elapsed().as_secs_f64() * 1000.0;
            let _profiled_entities = $self.biology_profiler_population;
            $self
                .biology_profiler_state
                .record($category, _profiled_ms, _profiled_entities);
        } else {
            $body;
        }
    }};
...
impl PhylonApp {
    ...
    pub(crate) fn update_simulation(&mut self) -> (f64, f64) {
        ...
        // Shared per-tick population-count context for the P9.1b biology
        // profiler's "entities processed" figures — computed once, only
        // when the profiler is enabled, never on a normal run. See
        // `biology_profiler`'s module doc for why this is one shared count
        // rather than a category-specific filtered query.
        if self.biology_profiler_config.enabled {
            self.biology_profiler_population = self
                .world
                .ecs
                .query::<&metabolism::Metabolism>()
                .iter(&self.world.ecs)
                .count() as u64;
        }
        ...
        profiled!(self, BiologyCategory::Hormones, {
            self.world
                .ecs
        ...
        profiled!(self, BiologyCategory::Growth, {
        ...
        profiled!(self, BiologyCategory::Development, {
        ...
        profiled!(self, BiologyCategory::SpatialUpdate, {
            self.world
                .ecs
                .run_system_once(ecology::build_resource_grids_system);
        });
        profiled!(self, BiologyCategory::Sensing, {
        ...
        profiled!(self, BiologyCategory::BrainGather, {
        ...
        profiled!(self, BiologyCategory::GpuBrainDispatch, {
            if let (Some(gpu), Some(brain_compute)) =
                (self.gpu.as_ref(), self.brain_compute.as_mut())
            {
        ...
        profiled!(self, BiologyCategory::Behavior, {
        ...
        profiled!(self, BiologyCategory::Metabolism, {
            self.world
                .ecs
        ...
        profiled!(self, BiologyCategory::Ecology, {
        ...
        profiled!(self, BiologyCategory::Predation, {
        ...
        profiled!(self, BiologyCategory::Disease, {
            self.world
                .ecs
        ...
        profiled!(self, BiologyCategory::Immune, {
            self.world
                .ecs
        ...
        profiled!(self, BiologyCategory::Morphogenesis, {
        ...
        profiled!(self, BiologyCategory::Reproduction, {
            self.world
                .ecs
        ...
        profiled!(self, BiologyCategory::Events, {
            self.world
                .ecs
        ...
        profiled!(self, BiologyCategory::Cleanup, {
            // Opt-in, zero-cost when `PHYLON_MOTION_DIAGNOSTIC` is unset —
            // see `motion_diagnostic`'s module doc comment.
            self.world
                .ecs
        ...
        profiled!(self, BiologyCategory::Diffusion, {
                ...
                (self.gpu.as_ref(), self.diffusion_compute.as_mut())
            {
                ...
                // Corpses emit CO2
                let mut query_dead = self.world.ecs.query_filtered::<(
                    &physics::ParticleNode,
                    &metabolism::Metabolism,
                ), bevy_ecs::query::With<metabolism::Dead>>(
                );
        ...
        if self.biology_profiler_config.enabled {
            self.biology_profiler_state.end_tick();
        }
