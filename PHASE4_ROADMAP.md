...
| P4-F3 | ~~Intra-body transport: a per-organism graph-flow pass moving resources along Body Graph/`Spring` edges (circulation, respiration, digestion, nutrient transport)~~ | 2 | F2 | High | No (additive system) — **Done** |
...
| P4-F3 — Intra-Body Transport | Re-audited `metabolism::ChemicalEconomy`/`metabolism_system` (unchanged since P4-F2) and `physics::Spring`/`physics::spring_force_system` (to confirm the Body Graph's parent/child edges are the same edges the physics solver already treats as structural connections — they are: `growth_system` builds one `Spring` per graph edge, so this milestone reuses an existing topological relationship rather than inventing a new one) before writing anything. **Implementation:** new module `crates/organisms/src/transport.rs`, exporting `transport_system`. For every organism's persistent `DevelopmentalGraph`, the system (1) collects `(parent_entity, child_entity)` pairs from `nodes[i].parent`, skipping any node whose own or parent's `entity` is `None` (a graph built by `simulate_growth_timeline`, or a pruned/apoptotic position); (2) snapshots each referenced entity's `ChemicalEconomy` once (missing components — e.g. an entity despawned by a broken `Spring` — are silently skipped, not treated as an error); (3) relaxes each edge in the graph's own parent-before-child insertion order via `relax_toward_equilibrium` — `new = old + (target - old) * TRANSPORT_RATE` (0.15), mass-conserving and capacity-clamped so a transfer can never push a pool over its max or below zero; (4) writes the final snapshot back. Only glucose/o2/atp move — co2/waste is explicitly deferred to P4-F6 per the roadmap's own epic boundary (documented in the module's doc comment, not silently expanded here). Registered in `crates/app/src/simulation.rs` immediately before `metabolism::metabolism_system`, so resources gained this tick (foraging/photosynthesis, which run earlier in the tick) can reach a segment before that segment's pool is respired from — a one-tick-lag-free ordering choice, unlike the GPU brain/physics dispatch's deliberate one-tick lag. **No `rayon`, no RNG:** per-organism edge lists are small (`MAX_SEGMENTS`-bounded) and processed sequentially in a fixed order derived from growth order, not thread scheduling — sidestepping the determinism hazard `metabolism_system` handles via its snapshot/parallel/reduce split, since transport's per-organism workload doesn't need parallelism to be fast enough. Added 6 tests: 2 for `relax_toward_equilibrium` itself (mass conservation; capacity/negative-value bounds), and 4 for `transport_system` end-to-end — resources actually move from a full head to an empty segment; a node with `entity: None` produces no edges and is a no-op (not a panic); a 3-node chain (head→torso→tail) propagates glucose two hops in a single tick (proving the sequential-relaxation design works as documented, not just in theory); and a repeated-run determinism check (identical starting state ⇒ bit-identical output), satisfying §9's "same-seed-same-output determinism tests for the new transport pass" requirement. | `cargo build --workspace --all-targets` clean; `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt --all -- --check` clean (one auto-fmt pass, reverified); `cargo test --workspace` all passing, 0 failed (organisms: 37 tests, +6 new); `RUSTDOCFLAGS="-D warnings" cargo doc -p organisms -p app --no-deps --document-private-items` clean. |

**No deviation from the roadmap found or needed for P4-F3** — implementation matches the milestone's stated goal (a per-organism graph-flow pass along Body Graph/`Spring` edges) with one explicit scope narrowing (co2/waste deferred to P4-F6), which is itself dictated by the roadmap's own epic split rather than a new decision. No new ADR was warranted: this is a new *system*, not a new architectural pattern — it consumes P4-F2's per-segment pools and ADR-P4-01's persistent graph exactly as both were designed to be built upon.

**Risks/technical debt introduced by this milestone:**

- `TRANSPORT_RATE = 0.15` is an untuned placeholder, same status as P4-F2's pool sizes — expect revisiting once there's a biological target to tune against (e.g. observed starvation rates at the tips of long body plans).
- The population-scale benchmark flagged as outstanding in P4-F2's risk notes has still not been run. This milestone adds real per-tick computation (not just static memory) for the first time, making that benchmark more worth doing soon than it was after P4-F2 alone — noted here rather than deferred silently again.
- `transport_system`'s edge-collection pass is `O(total segments across all organisms)` per tick, with an inner `HashMap` for the snapshot — fine at current population scales (small `MAX_SEGMENTS`), would need revisiting only alongside the same population-scale benchmark above.
- Per-segment pools are no longer fully inert (as P4-F2 flagged), but nothing yet *consumes* them to produce visible organism behavior beyond the pools' own numbers — e.g. no per-segment health/death consequence exists for a starved segment. That's implicitly future physiology work (P4-F4/F5/F6 and beyond), not part of this milestone's scope.

**Remaining work:** Per §6's recommended order, P4-F4 (endocrine signalling extension), P4-F5 (immune response), and P4-F6 (waste products) are next — any order among them per the roadmap, all gated behind this milestone. Per the approval's standing instruction, stopping here for review before beginning the next one.
