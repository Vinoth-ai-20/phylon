...
| M2 | Crossover/mutation for `regulatory_cppn` (reuse `Cppn`'s existing machinery) | M1 | Low | No (additive to M1's break) |
| M3 | Analytic morphogen gradient functions (AP position, distance-from-head) as `RegulatoryNetwork` inputs | M1 | Low | No |
...
**Remaining work:** M2 (crossover/mutation) is next, pending your review of this report.
